...
For an introduction to the analysis and visualization of population structure with NetView, see the [Pearl Oyster Tutorial](https://github.com/esteinig/netview/blob/master/tutorials/PearlOysterTutorial.md). Please note that the method is primarily designed for the **visualization** of population structure and does not have the same rigorous statistical backbone as other model-based approaches. It is therefore recommended to use the networks as a tool to understand and visualize genomic data across the genetic structure inferred by programs such as Admixture, DAPC or STRUCTURE.  We are expanding the scope of this package to better accommodate this approach, including novel workflows to detect key contributors, visualization of admixture proportions and geospatial mapping of samples and population networks.
